...
var Seeder = function () {
  ...
  _createClass(Seeder, [{
    ...
    value: function compileTemplate(template) {
      return (0, _objectFromTemplate2.default)(template, _faker2.default, this.opts);
    }
  }, {
    key: 'printDebug',
    ...
    value: function seedApp() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this.printDebug('Running ' + promises.length + ' seeder(s)...');

        return Promise.all(promises).then(function (seeded) {
          _this.printDebug('Created ' + seeded.length + ' total items:', seeded);
    ...
    value: function seed(cfg) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        return deletePromise.then(function (deleted) {
          ...
          if (cfg.template && cfg.disabled !== true) {
            ...
            if (randomize) {
              for (var _i = 0; _i < count; _i++) {
                var template = cfg.templates[Math.floor(Math.random() * cfg.templates.length)];
          ...
          if (!promises.length) {
            _this2.printDebug('Seeder disabled for ' + cfg.path + ', not modifying database.');
            return resolve([]);
          } else {
